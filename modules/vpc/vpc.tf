...
resource "aws_subnet" "public-subnet" {
  ...
  tags = {
    Name     = "${var.public-subnets-name}-${count.index + 1}-web"
...
resource "aws_subnet" "private-subnets" {
  ...
  tags = {
    Name     = "${var.private-subnet-name}-${count.index + 1}-app"
...
resource "aws_subnet" "private-subnet-RDS" {
  ...
  tags = {
    Name     = "${var.private-subnet-name}-RDS"
  }
}

#CREATING 2nd PRIVATE SUBNET FOR RDS
resource "aws_subnet" "private-subnet-2-RDS" {
  ...
  tags = {
    Name     = "${var.private-subnet-name}-1-RDS"
  }
}
